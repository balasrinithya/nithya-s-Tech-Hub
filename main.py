...
main = tkinter.Tk()
main.title("Electricity Fraud Detection with Deep Learning and Random Forests") 
...
def uploadDataset():
    ...
    pt = Table(frame, dataframe=dataset, showtoolbar=True, showstatusbar=True)
    pt.show()
    root.mainloop()
...
def preprocessDataset():
    ...
    root = Tk()
    root.title("Preprocessed Dataset")
    frame = Frame(root)
    frame.pack(fill='both', expand=True)
    pt = Table(frame, dataframe=pd.DataFrame(dataset), showtoolbar=True, showstatusbar=True)
    pt.show()
    root.mainloop()
...
def runCNN():
    ...
    if os.path.exists('model/model.json'):
        ...
        cnn_model.add(Dense(256, input_dim=X.shape[1], activation='relu', kernel_initializer = "uniform")) 
        cnn_model.add(Dense(128, activation='relu', kernel_initializer = "uniform"))
        cnn_model.add(Dense(Y.shape[1], activation='softmax',kernel_initializer = "uniform")) 
    ...
    results = {'Precision': [p], 'Recall': [r], 'F1 Score': [f], 'Accuracy': [a]}
    root = Tk()
    root.title("CNN with Random Forest Results")
    frame = Frame(root)
    frame.pack(fill='both', expand=True)
    pt = Table(frame, dataframe=pd.DataFrame(results))
    pt.show()
    root.mainloop()
...
def predict():
    ...
    for i in range(len(predict)):
        if predict[i] == 1:
            text.insert(END,str(data[i])+" ===> record detected as ELECTRICITY FRAUD\n\n")
        if predict[i] == 0:
            text.insert(END,str(data[i])+" ===> record NOT detected as ELECTRICITY FRAUD\n\n")     
...
title = Label(main, text='Electricity Fraud Detection', justify=LEFT)
title.config(bg='white', fg='darkslategray')  
...
font1 = ('times', 13, 'bold')
uploadButton = Button(main, text="Upload Electricity Dataset", command=uploadDataset)
uploadButton.place(x=100,y=150)
uploadButton.config(font=font1)

preprocessButton = Button(main, text="Preprocess Dataset", command=preprocessDataset)
preprocessButton.place(x=100,y=225)
preprocessButton.config(font=font1) 

cnnButton = Button(main, text="Generate CNN Model", command=runCNN)
cnnButton.place(x=100,y=300)
cnnButton.config(font=font1)

rfButton = Button(main, text="Run Random Forest", command=runRandomForest)
rfButton.place(x=100,y=375)
rfButton.config(font=font1)

#svmButton = Button(main, text="Run SVM Algorithm", command=runSVM)
#svmButton.place(x=200,y=245)
#svmButton.config(font=font1)

#cnnsvmButton = Button(main, text="CNN with SVM", command=runCNNSVM)
#cnnsvmButton.place(x=500,y=180)
#cnnsvmButton.config(font=font1)

cnnrfButton = Button(main, text="CNN-RF model", command=runCNNRF)
cnnrfButton.place(x=100,y=450)
cnnrfButton.config(font=font1)


predictButton = Button(main, text="Detect Electricity Fraud", command=predict)
predictButton.place(x=100,y=525)
predictButton.config(font=font1)

#graphButton = Button(main, text="Comparison Graph", command=graph)
#graphButton.place(x=100,y=575)
#graphButton.config(font=font1)

                            

font1 = ('times', 12, 'bold')
text=Text(main,height=50,width=300)
scroll=Scrollbar(text)
text.configure(yscrollcommand=scroll.set)
text.place(x=500,y=108)
